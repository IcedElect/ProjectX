...
var (
	ErrUnsupportedPaymentMethod = errors.New("unsupported payment method") //неподдерживаемый способ оплаты
)
...
func (e *InsufficientFundsError) Error() string { //метод нашей ошибки нехватки средств
	return fmt.Sprintf("Запрошенная сумма: %.2f, максимально допустимая сумма: %.2f", e.RequestedAmount, e.MaxAmount)
}
